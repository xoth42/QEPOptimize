...
using QEPO.Configurable # import AbstractConfiguration type from Configurable.jl
...
export generate_noisy_BellSwap_ops_for_individual, long_range_entanglement_generation!, Population, generate_valid_pairs,NoisyBellSwap,get_individuals,get_ops, initialize_pop_with_constraints!
...
function reset_population!(population,config)
        set_individuals!(population,[
            Individual("random", [], Performance([], 0, 0, 0, 0), 0.0)
            for _ in 1:get_population_size(config) * get_starting_pop_multiplier(config)
        ])
end
...
function calculate_performance!(indiv::Individual, config::AbstractConfiguration)
    ...
    initial_noise_circuit = [PauliNoiseOp(i, f_in_to_pauli(get_communication_fidelity_in(config))...) for i in 1:num_registers]
    ...
    marginals = counts_marginals / count_success # marginal fidelities of individial purified pairs
...
function long_range_entanglement_generation!(population::Population,config::AbstractConfiguration)
    ...
    reset_population!(population,advanced_config)
    num_registers = get_num_registers(config)
...
function initialize_pop_with_constraints!(population::Population, config::AbstractConfiguration)
    advanced_config = get_advanced_config(config)
    hardware_config = get_hardware_config(config)
    calibration_data = get_calibration_data(hardware_config)
    valid_qubits = get_valid_qubits(hardware_config)
...
# function run_with_constraints!(population::Population, calibration_data::Dict, valid_qubits::Array{Int}) # TODO from Stefan: the fact that there is `run` but also `run_with_constraints` kinda sounds like this can be written more neatly and simply if we use "multiple dispatch"

#     for hist in ["manual", "survivor", "random", "child", "drop_m", "gain_m", "swap_m", "ops_m"] # TODO from Stefan: this list gets repeated frequently, probably it makes sense to put it in a "convenience" global variable (and as mentioned elsewhere, probably a list of symbols, not a list of strings)
#         population.selection_history[hist] = Vector{Int64}()
#     end
#     initialize_pop_with_constraints!(population, calibration_data, valid_qubits)
#     sort!(population)  # Evaluate the performance and Sorts the individuals by fitness, placing the best-performing individuals at the top
#     cull!(population)  # Removes excess individuals to maintain the target population size
...
#         step_with_constraints!(population, calibration_data, valid_qubits)
...
#         performances = tmap(indiv -> calculate_performance!(indiv,  population.num_simulations), population.individuals)
#     end
# end

# # alternative one when printing out the history
# function run_with_constraints_history!(population::Population, calibration_data, valid_qubits)
#     max_purified_fidelities_gen_dic = zeros(population.max_gen,1)
#     min_purified_fidelities_gen_dic = zeros(population.max_gen,1)
#     ave_purified_fidelities_gen_dic = zeros(population.max_gen,1)

#     for hist in ["manual", "survivor", "random", "child", "drop_m", "gain_m", "swap_m", "ops_m"] # TODO from Stefan: these should probably be symbols, not strings, e.g. "drop_m" should be :drop_m
#         population.selection_history[hist] = Vector{Int64}()
#     end
#     initialize_pop_with_constraints!(population, calibration_data, valid_qubits)
#     sort!(population)
...
#         generations = 1:population.max_gen
...
# function step_with_constraints!(population::Population, calibration_data::Dict, valid_qubits::Array{Int})
...
#     # Generate children from each pair
...
#         population.individuals = vcat(population.individuals, [drop_op_with_constraints!(indiv) for i = 1:population.mutants_per_individual_per_type if rand() < population.p_lose_operation && length(indiv.ops) > 0])
#         # Adds a new operation, but only if the individual hasn’t reached the maximum number of operations
#         population.individuals = vcat(population.individuals, [gain_op_with_constraints!(indiv, calibration_data, valid_qubits) for i = 1:population.mutants_per_individual_per_type if rand() < population.p_add_operation && length(indiv.ops) < population.max_ops])
#         # Swaps two operations within an individual’s circuit
#         population.individuals = vcat(population.individuals, [swap_op_with_constraints!(indiv) for i = 1:population.mutants_per_individual_per_type if rand() < population.p_swap_operations && length(indiv.ops) > 0])
#         # Randomly modifies the operations of an individual
#         population.individuals = vcat(population.individuals, [mutate_with_constraints!(indiv, calibration_data, valid_qubits) for i = 1:population.mutants_per_individual_per_type if rand() < population.p_mutate_operations && length(indiv.ops) > 0])
#     end
...
# function new_child(indiv::Individual, indiv2::Individual, max_ops::Int,config::AbstractConfiguration)::Individual
...
# function swap_op_with_constraints!(indiv::Individual)
#     new_indiv = deepcopy(indiv)

#     """ Randomly select two positions (cnot or measurement) """
#     ind1 = rand([i for i in 1:length(new_indiv.ops) if isa(new_indiv.ops[i],PauliNoiseBellGate{CNOTPerm} ) || isa(new_indiv.ops[i], NoisyBellMeasureNoisyReset)])
#     ind2 = rand([i for i in 1:length(new_indiv.ops) if isa(new_indiv.ops[i],PauliNoiseBellGate{CNOTPerm} ) || isa(new_indiv.ops[i], NoisyBellMeasureNoisyReset)])
...
# function drop_op_with_constraints!(indiv::Individual)
#     new_indiv = deepcopy(indiv)

#     # Filter the indices of operations that can be dropped
#     drop_indices = [i for i in 1:length(new_indiv.ops) if isa(new_indiv.ops[i],PauliNoiseBellGate{CNOTPerm} ) || isa(new_indiv.ops[i], NoisyBellMeasureNoisyReset)]
...
# function gain_op_with_constraints!(indiv::Individual,  calibration_data::Dict, valid_qubits::Array{Int})
#     new_indiv = deepcopy(indiv)
...
#         q = rand(indiv.purified_pairs+1 : indiv.num_registers)          # debug
#         q_pair = map_num_to_valid_qubits(q, valid_pairs)
...
#                 rand(BellMeasure, q),
#                 pair_readout_error(calibration_data[q_pair[1]][:readout_error], calibration_data[q_pair[2]][:readout_error]),   #❓change and see difference
#                 f_in_to_pauli(population.f_in)...                              # only consider F_in, not consider T1, T2
#                 )
#         else
#         # Other registers are NoisyBellMeasureNoReset
#             NoisyBellMeasureNoisyReset(
...
#         i = new_indiv.r*(new_indiv.r-1)/2         # number of NoisyBellSwap gate is r*(r-1)/2
...
# """ applying the appropriate mutation function to each of its gates """
# function mutate_with_constraints!(indiv::Individual, calibration_data::Dict,  valid_qubits::Array{Int})::Individual
#     new_indiv = deepcopy(indiv)
#     # Apply mutation only to PauliNoiseBellGate{CNOTPerm} and NoisyBellMeasureNoisyReset operations
#     set_ops!(new_indiv, [isa(gate, PauliNoiseBellGate{CNOTPerm}) || isa(gate, NoisyBellMeasureNoisyReset) ? mutate(gate) : gate for gate in new_indiv.ops])
...
#     new_ops = [refresh_noise(op, f_in) for op in indiv.ops]
#     return Individual(indiv.history, indiv.purified_pairs, indiv.num_registers, f_in, indiv.code_distance, new_ops, Performance([], 0, 0, 0, 0), 0.0, indiv.optimize_for)
# end

# function total_raw_pairs(indiv::Individual)
#     total = indiv.num_registers
#     last_ops_reg = Set(1:indiv.num_registers)
...
#     return hash((Individual, indiv.purified_pairs, indiv.num_registers, [hash(op) for op in indiv.ops]))
# end
